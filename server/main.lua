...
local WeebhookLogo = nil
local WeebhookName = "LSPD"


-- Functions

function SendWebHook(LogType, title, color, message)
    timestamp = os.date("%c")
    local embedMsg = {}

    QBCore.Debug(LogType)
    QBCore.Debug(message)
    if LogType == nil then print(1)
    else
        if LogType == "Duty" then
            whLink = Config.DutyLogWebhook
             embedMsg = {
                {
                    ["title"] = title,
                    ["color"] = color,
                    ["description"] =        
                        "\n**Officer:** ".."*".. message.name.. "*" ..
                        "\n **Callsign: **"..message.callsign..
                        "**\n Clocked: **" .."*"..message.DutyStat.. "*",
                    ["footer"] ={
                        ["text"] = timestamp.." (Server Time).",
                    },
                }
            }
        elseif LogType == "armory" then
            whLink = Config.ArmoryWeebHook
        else
            embedMsg = {
                {
                    ["color"] = color,
        ...
        function(err, text, headers)end, 'POST', json.encode({username = WeebhookName, avatar_url= WeebhookLogo ,embeds = embedMsg}), { ['Content-Type']= 'application/json' })
    end
end
...
RegisterNetEvent('police:server:CreateLog', function(LogType, title, color, message)
    SendWebHook(LogType, title, color, message)
    print(1)
end)
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price) --ToDo
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
